<template>
          ...
          if (data.ok) {
            if (!checkData.exists) {
              ...
              const response = await fetch(
                "http://localhost:3005/api/mail/send-reset-password",
                {
                  method: "POST",
                  headers: {
                    "Content-Type": "application/json",
              ...
              if (response.ok) {
                ...
                toast.error(resData.message || "Gửi link đặt lại mật khẩu thất bại.");
              }
            }
          } else {
  ...
  @media (min-width: 769px) {
    .form-container {
      margin-top: 150px;
      box-shadow: 0 8px 40px 0 rgba(0,0,0,0.14), 0 2px 12px rgba(0,0,0,0.10);
    }
  }
