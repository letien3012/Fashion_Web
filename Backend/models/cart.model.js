...
cartSchema.statics.updateCartItemQuantity = async function(cartId, productId, variantSku, quantity) {
  try {
    const cart = await this.findById(cartId);
    if (!cart) {
      throw new Error("Cart not found");
...
cartSchema.statics.removeFromCart = async function(cartId, productId, variantSku) {
  try {
    const cart = await this.findById(cartId);
    if (!cart) {
      throw new Error("Cart not found");
