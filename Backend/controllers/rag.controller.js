...
ragController.embedding = async (req, res) => {
  try {
    ...
    const response = await axios.post('http://localhost:9000/vectorize-text', { text: question });
    if (response.data && response.data.embedding) {
      return res.json({ embedding: response.data.embedding });
    } else {
      return res.status(500).json({ message: 'Invalid response from YOLO service' });
    }
  } catch (error) {
    console.error('Error calling YOLO service for text embedding:', error.message);
    return res.status(500).json({ message: `Failed to create text embedding: ${error.message}` });
  }
};
...
ragController.retrieve = async (req, res) => {
  try {
    ...
    if (!embedding || !Array.isArray(embedding)) return res.status(400).json({ message: 'Missing or invalid embedding' });
